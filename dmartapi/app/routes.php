	...
	*/
	...
	$app->get("$burl/prodinfo/{field}/{srch}", function (Request $request, Response $response, $args) {
        try {
			...
			$srch=$args['srch'];
			$db=getconn();
			$sql="select code id,name text,Category,Descn,Type,Packing,Unit,MfgComp,Salt,
			MinOrdQty,PurchaseRate,MRP,RateB,RateC,Discount,MinStock,MaxStock,Narcotic,
	...
	$app->post("$burl/purimport", function (Request $request, Response $response) {
		try {
			...
			$sql = "INSERT INTO purreggrn(invoiceno,invoicedate,invoiceamt,updinvoiceamt,suppid,discount,netamt,branchid,remarks,noteno) 
			VALUES(:invoiceno,:invoicedate,:invoiceamt,:updinvoiceamt,:suppid,:discount,0,:branchid,:remarks,:noteno)";
			$db = getconn();
			// $db->beginTransaction();
			$stmt = $db->prepare($sql);
			$stmt->bindParam(":invoiceno", $inv['invoiceno']);
			...
			$stmt->bindParam(":discount", $inv['discount']);
			$stmt->bindParam(":branchid", $inv['branchid']);			
			$stmt->bindParam(":updinvoiceamt", $inv['updinvoiceamt']);			
			$stmt->bindParam(":remarks", $inv['remarks']);			
			$stmt->bindParam(":noteno", $inv['noteno']);			
			$stmt->execute();
			...
			if ($inv['diffAmt'] > (0.55)) 
			{	
				$sql = "INSERT INTO purr_adjust_amt(branchid,supplier_id,invoiceamt,adjust_amt,paid,noteno) 
				VALUES(:branchid,:supplier_id,:invoiceamt,:adjust_amt,0,:noteno)";
				$db = getconn();
				// $db->beginTransaction();
				$stmt = $db->prepare($sql);
				$stmt->bindParam(":branchid", $inv['branchid']);
				$stmt->bindParam(":supplier_id", $inv['suppid']);
				$stmt->bindParam(":invoiceamt", $inv['invoiceamt']);
				$stmt->bindParam(":adjust_amt", $inv['diffAmt']);
				$stmt->bindParam(":noteno", $impnote);
				$stmt->execute();
			}

			if ($inv['invoiceamt'] !==$inv['updinvoiceamt']) 
			{
				$sql = "UPDATE purr_adjust_amt SET paid=1 WHERE supplier_id=:supplier_id AND noteno=:noteno AND branchid=:branchid";
				$db = getconn();
				$stmt = $db->prepare($sql);
				$stmt->bindParam(":branchid", $inv['branchid']);
				$stmt->bindParam(":supplier_id", $inv['suppid']);
	...
	// Begin REST API for Received Items Entity from Branch
	$app->post("$burl/receiveitem", function (Request $request, Response $response) {
		try {
			//$body = $request->getBody();
			$inv = $request->getParsedBody();
			
			$sql = "UPDATE transfer SET transferglbid=CONCAT('DMT', '', `transferid`), transferstatus='Transferred' where id=:id";
			$db = getconn();
			$stmt = $db->prepare($sql);
	...
	$app->post("$burl/tranferstk", function (Request $request, Response $response) {
		try {
			...
			$db->exec("UPDATE transfer SET netamt='".$inv['netamt']."' where transferid='$transferid'");
			// $db->exec("UPDATE purreggrn SET netamt='".$inv['netamt']."' where grnno='$grnno'");
			$db->commit();
			// if ($stmt->rowCount()>0) $msg="success"; else $msg="no update";
			$db = null;$status=201;
...
$app->post("$burl/retailsaleinv", function (Request $request, Response $response) {
	try {
		...
		$sql = "INSERT INTO saleregbill(date,patientid,doctorid,paymentmode,discount,netamount,actualamt,servicecharge,
				othercharge,cgstamt,sgstamt,igstamt,userid, branchid) VALUES(:date,:patientid,:doctorid,NULL,:discount,:netamount,0,0,0,0,0,0,1,:branch)";
		$db = getconn();
		$stmt = $db->prepare($sql);
		...
		$stmt->bindParam(":discount", $sinv['discount']);
		...
		$sql = "INSERT INTO saleregprod(prodcode,qty,batchno,amount,expdate,billno,cgstp,sgstp,igstp,discountper,discountamt,price,branchid) 
			VALUES(:prodcode,:qty,:batchno,0,:expdate,:billno,:cgstp,0,0,:discountper,:discountamt,:price,:branch)";
		$db = getconn();
		$stmt = $db->prepare($sql);
...
/////End Retail sale Invoice//////
	...
	// START REST API for Retail Sale Info
        ...
        try {
			...
			/*$sql="select code id, name text, category, mstock.qty, mstock.batchno, mstock.purchasecost, mstock.mrp, mstock.pcsrate, mstock.expdate, purregprod.cgstp from druginfo 
			       inner join purregprod on druginfo.Code=purregprod.prodcode 
				  inner join mstock on druginfo.Code=mstock.drugcode and mstock.qty>0 and mstock.branchid=$branch";
			*/
			$sql="select code id, name text, category from druginfo";
			if ($srch)
				$sql.=" where $field like '%$srch%' GROUP BY druginfo.Code, druginfo.Name";
			$sql.=" limit 20";
			$result=$db->query($sql);
			$data=$result->fetchAll(PDO::FETCH_ASSOC);
			$status=200;
	...
	///// End REST API for Retail Sale Info/////


// START REST API for Retail Sale Batch Info
	
$app->group("$burl/retailbatchinfo", function (Group $group) {

	$group->get("/{prodid}", function (Request $request, Response $response, $args) {
		try {
			$db = getconn();
			$params = $request->getQueryParams();
		$branch=$params['branch'];
			
			$result = $db->query("select distinct mstock.batchno, mstock.qty, mstock.mrp, mstock.pcsrate, mstock.expdate from druginfo inner join mstock on druginfo.Code=mstock.drugcode and mstock.qty>0 where mstock.branchid='$branch' and mstock.expdate>CURDATE() and mstock.drugcode = " . $args['prodid']);
			$data = $result->fetchAll(PDO::FETCH_ASSOC);
			$status = 200;
		} catch (Exception $e) {
	...
	///// End REST API for Retail Sale Batch Info/////



 ///// Start API for stockreportinfo /////
 $app->get("$burl/stockreportinfo/{field}/{srch}", function (Request $request, Response $response, $args) {
	try {
		$field=$args['field'];
		$srch=$args['srch'];
		$db=getconn();
		...
		$sql="select code id, concat(druginfo.name,' ',mstock.batchno,' #',mstock.qty) text,mstock.drugcode,druginfo.name,mstock.qty,mstock.pcsrate,mstock.mrp,mstock.batchno,mstock.expdate,druginfo.category,mfrcomp.name from druginfo 
			  join mfrcomp on  druginfo.mfgComp=mfrcomp.id
			  inner join mstock on druginfo.Code=mstock.drugcode";
		if ($srch)
			$sql.=" where $field like '%$srch%'";
		$sql.="limit 20";
		$result=$db->query($sql);
...
// End REST API for stockreportinfo
...
try {
	...
	$sql = "INSERT INTO saleorderbill(date,patientid,doctorid,paymentmode,discount,netamount,actualamt,servicecharge,
	...
	$branch=$sinv['branch'];
	$billno = $db->lastInsertId();
	$sql = "INSERT INTO saleorderprod(prodcode,qty,batchno,amount,expdate,billno,cgstp,sgstp,igstp,discountper,discountamt,price,branchid) 
		VALUES(:prodcode,:qty,:batchno,0,:expdate,:billno,0,0,0,0,0,:price,:branch)";
	$db = getconn();
	$stmt = $db->prepare($sql);
	...
	$db->exec("UPDATE saleorderbill SET netamount='".$sinv['netamount']."' where billno='$billno'");
	//$db->commit();
	//if ($stmt->rowCount()>0) $msg="success"; else $msg="no update";
	//$db = null;$status=201;
	//$data=array("item"=>$inv);
...
$response->getBody()->write(json_encode($data));
	...
	////// START REST API FOR SALE INFO //////
        ...
        try {
			...
			$sql="select code id,concat(name,' ',batchno) text, mstock.qty, mstock.batchno, mstock.MRP, mstock.pcsrate,mstock.expdate from druginfo 
	...
	$app->get("$burl/salereturn/{field}/{srch}", function (Request $request, Response $response, $args) {
        try {
			...
			saleregprod.price,saleregprod.prodcode pcode,patientdet.name customer,doctordet.name doctor,patientdet.id patientid,doctordet.id doctorid from saleregbill
	...
	$app->post("$burl/salereturn", function (Request $request, Response $response) {
		try {
			//$body = $request->getBody();
			$sinv = $request->getParsedBody();
			//$sql= "SELECT cgstp, cgstamt, sgstp, sgstamt, igstp, igstamt FROM purregprod where prodcode=':prodcode' and batchno=':batchno'";
			...
			$sql = "INSERT INTO saleretregbill(oldbillno,date,patientid,doctorid,paymentmode,discount,netamount,actualamt,servicecharge,
			        othercharge,cgstamt,sgstamt,igstamt,userid, branchid) VALUES(:oldbillno,:date,:patientid,:doctorid,:paymentmode,:discount,:netamount,:actualamt,0,0,0,0,0,1,:branch)";
			$db = getconn();
			$stmt = $db->prepare($sql);
			
			$stmt->bindParam(":date", $date);
			$stmt->bindParam(":patientid", $sinv['patientid']);
			$stmt->bindParam(":oldbillno", $sinv['oldbillno']);
			$stmt->bindParam(":doctorid", $sinv['doctorid']);
			$stmt->bindParam(":paymentmode", $sinv['paymentmode']);
			$stmt->bindParam(":netamount", $sinv['netamount']);
			$stmt->bindParam(":actualamt", $sinv['actualamt']);
			$stmt->bindParam(":discount", $sinv['discount']);
			$stmt->bindParam(":branch", $sinv['branch']);
			...
			$branch=$sinv['branch'];
			$billno = $db->lastInsertId();
			$sql = "INSERT INTO saleretregprod(prodcode,qty,batchno,amount,expdate,billno,cgstp,sgstp,igstp,discountper,discountamt,price,branchid) 
				VALUES(:prodcode,:qty,:batchno,:amount,:expdate,:billno,:cgstp,0,0,:discountper,:discountamt,:price,:branch)";
			$db = getconn();
			$stmt = $db->prepare($sql);
			...
			$db->exec("UPDATE saleretregbill SET netamount='".$sinv['netamount']."' where billno='$billno'");
			//$db->commit();
			//if ($stmt->rowCount()>0) $msg="success"; else $msg="no update";
			//$db = null;$status=201;
			//$data=array("item"=>$inv);
	...
	//Begin REST API for Purchase History View entity
	$app->group("$burl/purhistview", function (Group $group) {

		$group->get("/{id}", function (Request $request, Response $response, $args) {
			try {
				$db = getconn();
				$result = $db->query('SELECT purreggrn.grnno, druginfo.Name, purregprod.packing, purregprod.qty, purregprod.batchno, purregprod.purrate, purregprod.mrp, purregprod.expdate, purregprod.cgstp, purregprod.total from purregprod
									INNER JOIN purreggrn ON purreggrn.grnno=purregprod.grnno
									INNER JOIN druginfo ON druginfo.Code=purregprod.prodcode where purreggrn.grnno = ' . $args['id']);
				$data = $result->fetchAll(PDO::FETCH_ASSOC);
				$status = 200;
			} catch (Exception $e) {
				$data = array("errmsg" => $e->getMessage());
	...
	//End of REST API for Purchase History View entity
	
	// Begin REST API for purchase Return Entity
	$app->post("$burl/purreturn", function (Request $request, Response $response) {
		try {
			//$body = $request->getBody();
			$inv = $request->getParsedBody();
			...
			$sql = "INSERT INTO purretreg(discount,grndate,invoiceamt,invoicedate,invoiceno,itemrecvdate,netamt,suppid,branchid,retgrnno) 
			VALUES(:discount,:grndate,:invoiceamt,:invoicedate,:invoiceno,:itemrecvdate,:netamt,:suppid,:branchid,:retgrnno)";
			$db = getconn();
			// $db->beginTransaction();
			$stmt = $db->prepare($sql);
			$stmt->bindParam(":discount", $inv['discount']);
			$stmt->bindParam(":grndate", $inv['invoicedate']);
			$stmt->bindParam(":invoiceamt", $inv['invoiceamt']);
			$stmt->bindParam(":invoicedate", $inv['invoicedate']);
			$stmt->bindParam(":invoiceno", $inv['invoiceno']);
			$stmt->bindParam(":itemrecvdate", $inv['invoicedate']);
			$stmt->bindParam(":netamt", $inv['total']);
			$stmt->bindParam(":suppid", $inv['suppid']);
			$stmt->bindParam(":branchid", $inv['branchid']);
			$stmt->bindParam(":retgrnno", $inv['grnno']);
			$stmt->execute();

			// Get the last inserted ID for the GRN
			$grnno = $db->lastInsertId();
			$returnnote = 'RET' . $grnno;
			...
			{	
				...
				$stmt->bindParam(":supplier_id", $inv['suppid']);
				$stmt->bindParam(":invoiceamt", $inv['invoiceamt']);
				$stmt->bindParam(":adjust_amt", $inv['netamt']);
				$stmt->bindParam(":noteno", $returnnote);
				$stmt->execute();
			}

			$branchid=$inv['branchid'];

			$sql = "INSERT INTO purretprod(grnno,prodcode,qty,free,batchno,purrate,mrp,expdate,cgstp,cgstamt,packing,total,branchid,eqty,uqty) 
					VALUES(:grnno,:prodcode,:qty,:free,:batchno,:purrate,:mrp,:expdate,:cgstp,:cgstamt,:packing,:total,:branchid,:eqty,:uqty)";
			$db = getconn();
			...
			foreach($inv['items'] as $item) {
				...
				$stmt->bindParam(":total", $item['total']);
				$stmt->bindParam(":grnno", $grnno);
				$stmt->bindParam(":uqty", $item['uqty']);
				$stmt->bindParam(":eqty", $item['eqty']);
				$stmt->execute();
			}
				
			$db->exec("UPDATE purretreg SET netamt='".$inv['netamt']."' where grnno='$grnno'");
			// $db->commit();
			// if ($stmt->rowCount()>0) $msg="success"; else $msg="no update";
			// $db = null;$status=201;
	...
	// End REST API for purchase Return Entity
	...
	$app->get("$burl/stocktrnfer/{field}/{srch}", function (Request $request, Response $response, $args) {
        try {
			...
			$params = $request->getQueryParams();
			$branch=$params['branch'];
			$sql="select code id,concat(name,' ',ms.qty) text,ms.purchasecost,ms.packing,ms.cgst,ms.qty,ms.discount,ms.batchno,di.MRP,ms.pcsrate,ms.expdate from druginfo di join mstock ms on di.Code=ms.drugcode and branchid=$branch";
			if ($srch)
				$sql.=" where $field like '%$srch%'";
			$sql.=" limit 20";
			$result=$db->query($sql);
	...
	//Begin REST API for Sale Order Transfer Select entity
	$app->group("$burl/sotrans", function (Group $group) {
		$group->post('', function (Request $request, Response $response, $args) {
			try {
				...
				$db = getconn();
				$stmt = $db->prepare('SELECT  billno,date,patientid,name FROM saleorderbill srb JOIN patientdet pd ON srb.patientid=pd.id WHERE branchid = :branch AND date BETWEEN :fromdt AND :todt');
	...
	//End of REST API for Sale Order Transfer Select entity
      
	  
	 // Begin REST API for Sale Order Received Items Entity from Branch
	$app->post("$burl/soreceiveitem", function (Request $request, Response $response) {
		try {
			//$body = $request->getBody();
			$inv = $request->getParsedBody();
			
			$sql = "UPDATE sotransfer SET transferglbid=CONCAT('DMT', '', `transferid`), transferstatus='Transferred' where id=:id";
			$db = getconn();
			$stmt = $db->prepare($sql);
	...
	// End REST API for Sale Order Received Items Entity

	//Begin REST API for transferHistoryView entity
	$app->group("$burl/sotransferHistView", function (Group $group) {

		$group->get("/{id}", function (Request $request, Response $response, $args) {
			try {
				$db = getconn();
				$result = $db->query('select di.Name, sots.qty, sots.batchno, sots.expdate,sots.mrp, di.Category from sotransferstk sots join druginfo di on sots.drugcode = di.Code where sots.transferid = ' . $args['id']);
				$data = $result->fetchAll(PDO::FETCH_ASSOC);
				$status = 200;
			} catch (Exception $e) {
				$data = array("errmsg" => $e->getMessage());
	...
	$app->group("$burl/sotransferView", function (Group $group) {
		...
		$group->get("/{id}", function (Request $request, Response $response, $args) {
			try {
				$db = getconn();
				$result = $db->query('select di.Name,sop.prodcode,sop.qty,sop.batchno,sop.price,sop.expdate,
									sop.cgstp,sop.amount,sop.billno,sob.netamount,sob.patientid,sob.branchid 
									from saleorderprod sop 
									join druginfo di on sop.prodcode = di.Code 
									join saleorderbill sob on sop.billno=sob.billno 
									where sop.billno = ' . $args['id']);
				$data = $result->fetchAll(PDO::FETCH_ASSOC);
				$status = 200;
			} catch (Exception $e) {
				$data = array("errmsg" => $e->getMessage());
	...
	$app->group("$burl/sotransfer", function (Group $group) {
		$group->post('', function (Request $request, Response $response, $args) {
			try {
				...
				$transferid=$db->query("SELECT max(transferid)+1 from sotransfer")->fetch()[0];
				
				$transferdate = date('Y-m-d H:i:s');
				$transferglbid = 'DMT' . $transferid;
	
				$sql = "INSERT INTO sotransfer(transferdate, netamt, actualamt, sendingbrnid, receivebrnid, patientid, billno,transferid,transferstatus) 
						VALUES(:transferdate, :netamt, :actualamt, :sendingbrnid, :receivebrnid, :patientid, :billno,:transferid,'Pending')";

				$stmt = $db->prepare($sql);
				$stmt->bindParam(":transferdate", $transferdate);
				$stmt->bindParam(":netamt", $inv['netamt']);
				$stmt->bindParam(":actualamt", $inv['netamt']);
				$stmt->bindParam(":sendingbrnid", $inv['sendingbrnid']);
				$stmt->bindParam(":receivebrnid", $inv['branchid']);
				$stmt->bindParam(":patientid", $inv['patientid']);
				$stmt->bindParam(":billno", $inv['billno']);
				$stmt->bindParam(":transferid", $transferid);
				$stmt->execute();

				$soId=$db->lastInsertId();
				// Get the last inserted ID for the sotransfer
				$billno = $inv['billno'];
				$data = $inv['data'];
	
				// Inserting sotransfer items (if necessary)
				// Uncomment the code below if you have 'data' array in the $inv variable
	
				$sql = "INSERT INTO sotransferstk(drugcode,qty,mrp,batchno,expdate,amount,billno,transferid) 
						VALUES(:drugcode,:qty,:mrp,:batchno,:expdate,:amount,:billno,:transferid)";
				$stmt = $db->prepare($sql);
				$result = array(); // Store the inserted data
				foreach ($inv['data'] as $item) 
				{
					$stmt->bindParam(":drugcode", $item['drugcode']);
					$stmt->bindParam(":billno", $billno);
					$stmt->bindParam(":qty", $item['qty']);
					...
					$insertedItem = array(
						...
						"amount" => $item['amount'],
						"mrp" => $item['mrp'],
						"transferglbid"=>$transferglbid
					);
					$result[] = $insertedItem;
				}
				//$db->exec("UPDATE sotransfer SET transferglbid='$transferglbid' where id='$soId'");
  ...
  Location,gsttaxcat,gsthsncode,cgst,sgst,igst,ingredientid
			...
			*/
			$sql = "INSERT INTO druginfo(Name,Descn,Type,Packing,Unit,MinOrdQty,PurchaseRate,MRP,Discount,AvlQty,cgst,sgst,gst_type
			)VALUES(:name,:descn,:type,:packing,:unit,:MinOrdQty,:PurchaseRate,:mrp,:Discount,:AvlQty,:cgst,:sgst,:gst_type)";
			$db = getconn();
			$stmt = $db->prepare($sql);
			$stmt->bindParam(":name", $drug['name']);
			...
			$stmt->bindParam(":gst_type", $drug['type']);
			//$stmt->bindParam(":minordqty", $drug['minordqty']);
			$stmt->execute();
			if ($stmt->rowCount()>0) $msg="success"; else $msg="no update";
			$db = null;$status=201;
			$data=array("item"=>$drug);
			$data = array("status"=>"Ok","msg"=>"Inserted sucessfully","item"=>$data);
		} catch(Exception $e) {
			$data=array("status"=>"Error","msg"=>$e->getMessage());$status=200;
		}
		$response->getBody()->write(json_encode($data));
	...
	$app->group("$burl/bank", function (Group $group) {
		...
		try {
			...
			$sql = "INSERT INTO bank(name,address) 
				VALUES(:name,:address)";
			$db = getconn();
			$stmt = $db->prepare($sql);
			//$stmt->bindParam(":id", $drug['id']);
