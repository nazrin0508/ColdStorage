	...
	$app->get("$burl/retailsaleinfo/{field}/{srch}", function (Request $request, Response $response, $args) {
        try {
			...
			$sql="select code id, name text, category from druginfo
				  inner join mstock on druginfo.Code=mstock.drugcode and mstock.qty>0 and mstock.branchid=$branch";
			if ($srch)
				$sql.=" where $field like '%$srch%' GROUP BY druginfo.Name, druginfo.Category";
			$sql.=" limit 20";
			$result=$db->query($sql);
			$data=$result->fetchAll(PDO::FETCH_ASSOC);
			$status=200;
...
$app->group("$burl/retailbatchinfo", function (Group $group) {
	...
	$group->get("/{prodid}", function (Request $request, Response $response, $args) {
		...
		$branch=$params['branch'];
			...
			$result = $db->query("select distinct mstock.batchno, mstock.qty, mstock.mrp, mstock.pcsrate, mstock.expdate from druginfo
								inner join mstock on druginfo.Code=mstock.drugcode and mstock.qty>0 where mstock.branchid='$branch' and mstock.expdate>CURDATE() and code = " . $args['prodid']) ;
			$data = $result->fetchAll(PDO::FETCH_ASSOC);
			$status = 200;
		} catch (Exception $e) {
...
$response->getBody()->write(json_encode($data));
	...
	$app->get("$burl/stocktrnfer/{field}/{srch}", function (Request $request, Response $response, $args) {
        try {
			...
			$params = $request->getQueryParams();
			$branch=$params['branch'];
			$sql="select code id,concat(name,' ',ms.qty) text,ms.discount,ms.purchasecost,ms.packing,ms.cgst,ms.qty,ms.batchno,di.MRP,ms.pcsrate,ms.expdate from druginfo di join mstock ms on di.Code=ms.drugcode and branchid=$branch";
			if ($srch)
				$sql.=" where $field like '%$srch%'";
			$sql.=" limit 20";
			$result=$db->query($sql);
